...
function FileLoader(conf) {
    ...
    function walkSync(dir, filelist, subdir, key) {
        ...
        files.forEach(function (file) {
            if (fs.statSync(path.join(dir, file)).isDirectory()) {
                ...
                let filename = path.join(subdir, file);
                ...
                if (dir != ENTRY_DIR && key && Array.isArray(entry[key])) {
                    ...
                    if (ENTRY_FILES_EXT.some(ext => {
                        ...
                        const fKey = filename.substring(0, filename.length - cExt.length);
    ...
    // set entry
    configs.entry = configs.entry || entry;
    ...
    } else if (typeof configs.output === 'object') {
        if (!configs.output.path)
            configs.output.path = path.resolve(OUTPUT_DIR, "");
        if (!configs.output.filename)
            configs.output.filename = OUTPUT_FILES;
    }
